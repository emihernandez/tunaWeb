(function(window, angular, $) {
    ...
    angular.module('FileManagerApp').factory('item', ['$http', '$q', '$translate', 'fileManagerConfig', 'chmod', function($http, $q, $translate, fileManagerConfig, Chmod) {
        ...
        var Item = function(model, path, auditoriaId) {
            var rawModel = {
        ...
        Item.prototype.createFolder = function() {
            ...
            self.inprocess = true;
            self.error = '';
            $http.post(fileManagerConfig.createFolderUrl + '?id=' + self.tempModel.auditoriaId, data).success(function(data) {
        ...
        Item.prototype.download1 = function(preview) {
            var self = this;
            var data = {
                mode: "download",
                preview: preview,
        ...
        Item.prototype.download = function() {
            ...
            var queryObject = '';
            if(!fileManagerConfig.uploadFile){
              queryObject = {path: '/Comprobantes' + self.tempModel.fullPath(), id: self.tempModel.auditoriaId};
            }else{
              queryObject = {path: self.tempModel.fullPath(), id: self.tempModel.auditoriaId};
            }
            $http({
                url: fileManagerConfig.getContentUrl,
                method: "POST",
                params: queryObject,
                data: queryObject, 
                headers: {
               ...
               responseType: 'blob'
                ...
                var blob = new Blob([responseData], {type: data.type});                 
        ...
        Item.prototype.remove = function() {
            ...
            self.inprocess = true;
            self.error = '';
            $http.post(fileManagerConfig.removeUrl + '?id=' + self.tempModel.auditoriaId, self.tempModel.fullPath()).success(function(data) {
                self.deferredHandler(data, deferred);
            }).error(function(data) {
